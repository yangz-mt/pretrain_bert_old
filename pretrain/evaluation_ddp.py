...
def evaluate(model, args, logger, global_step):
    ...
    with torch.no_grad():
        ...
        for shard in range(start_shard, len(os.listdir(args.eval_data_path_prefix))):
            ...
            eval_log_str = (
                ...
                + f"| mins/batch: {elapsed_time_per_iteration :.3f} seconds | loss: {cur_loss:.7f} | ppl loss: {eval_ppl:.7f}"
            )
